...
layer2-exporter-service: {} # the service which handles exporting - no options available
...
layer2-metadata-service: {} # the service for metadata - no options available
layer3-research-manager: {} # storage for the internal research object - no options available
layer3-token-storage: {} # storage for oauth tokens - no options available
feature:
