...
class ExporterService:
    def __init__(self, testing=False, testing_address=None):
        ...
        if testing:
            self.testing_address = (
                "http://localhost:3000" if testing_address is None else testing_address
            )
    ...
    def export(self, from_service: str, to_service: str, filepath: str, user: str):
        ...
        url = (
            f"http://circle1-port-{to_service}"
            if not self.testing
            else self.testing_address
        )
        ...
        response_to = requests.post(
            f"{url}/deposition/{depositionId}/actions/upload",
            files=file,
            data={"userId": user},
            verify=(os.environ.get("VERIFY_SSL", "True") == "True"),
        )
        ...
        if (
            response_from is not None
            and response_to is not None
            and response_from.status_code < 300
            and response_to.status_code < 300
        ):
            return True
