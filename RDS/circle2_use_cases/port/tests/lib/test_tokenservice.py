...
class Test_TokenService(unittest.TestCase):
    ...
    def test_get_all_service_jwt(self):
        ...
        pact.given("no service was registered yet.").upon_receiving(
            "a request to get all services."
        ).with_request("GET", "/service").will_respond_with(
            200, body={"length": 0, "list": []}
        )
    ...
    def test_exchange_code(self):
        ...
        ).with_request("POST", f"/oauth2/token").will_respond_with(200, body=body)
        ...
        pact.given("no oauthservice was registered.").upon_receiving(
            ...
            500, body={"error": "ServiceNotExistsError", "http_code": 500}
        )
    ...
    def test_serviceprojects_projects_not_supported(self):
        ...
        ).with_request("GET", f"/metadata/project").will_respond_with(500, body="")
        with pact:
            projects = self.tokenService.getProjectsForToken(self.token1)
            self.assertEqual(projects, expected_projects)
    ...
    def test_createProjectForUserInService(self):
        ...
        pact.given("service without projects").upon_receiving(
            "try to create a project"
        ).with_request("POST", f"/metadata/project").will_respond_with(500, body="")
