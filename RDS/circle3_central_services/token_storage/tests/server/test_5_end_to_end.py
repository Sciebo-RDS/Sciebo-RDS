...
from RDS import User, OAuth2Service, Token, OAuth2Token
from RDS.Util import initialize_object_from_json
...
@unittest.skipUnless(
    os.getenv("CI_DEFAULT_BRANCH") is not None,
    "This should not be executed locally because we use selenium and firefox webdriver.",
)
class test_end_to_end(unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.getenv("ZENODO_OAUTH_CLIENT_SECRET") is not None,
        "This tests the zenodo oauth workflow, which only on protected branch will be tested.",
    )
